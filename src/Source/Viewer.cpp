...
Viewer::Viewer()
	: App(1024, 640)
	, m_framebuffer(window_width(), window_height())
	, m_saved_file("")
{
...
int Viewer::render_laplacian_params()
{
	...
	ImGui::RadioButton("OBJ", &m_save_as_obj, 1);
	ImGui::SameLine();
...
int Viewer::render_ui()
{
	...
	ImGui::GetWindowDrawList()->AddImage(
		(void*)static_cast<uintptr_t>(m_framebuffer.texture_id()),
		ImVec2(pos.x, pos.y),
...
int Viewer::render_demo_buttons()
{
	...
	{
		ImGui::SetTooltip(
			!m_delaunay_demo ? "Activate Delaunay triangulation demo." : "Delaunay triangulation demo is active.");
	}
	ImGui::EndDisabled();
