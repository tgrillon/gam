<<<<<<<< HEAD:GAM/Source/Mesh.cpp
#include "Mesh.h"
========
#include "mesh.h"
>>>>>>>> 649efc29359ee4c202c7508a5ce4001ea324a972:src/mesh.cpp
...
========
      file << "f ";
      file << (f.Vertices[0] + 1) << "//" << (f.Vertices[0] + 1) << " ";  
      file << (f.Vertices[1] + 1) << "//" << (f.Vertices[1] + 1) << " ";  
      file << (f.Vertices[2] + 1) << "//" << (f.Vertices[2] + 1) <<  "\n";
>>>>>>>> 649efc29359ee4c202c7508a5ce4001ea324a972:src/mesh.cpp
    }
  ...
  Vector TMesh::VertexNormal(IndexType iVertex) 
  {
    ...
    for (size_t j= 1; j <= numOfNeighbors; ++j)
    {
      ...
      normal.X+= (cotAlpha+cotBeta)*(uj.X-ui.X);
      normal.Y+= (cotAlpha+cotBeta)*(uj.Y-ui.Y);
      normal.Z+= (cotAlpha+cotBeta)*(uj.Z-ui.Z);
    }
    normal= normal/(2.*PatchAreaForVertex(iVertex));
